...
> 如果你从 Git 仓库下载的是 `cpe_test-v4.3.1-windows-config-docs.zip`，那是一份只含
> 配置、文档和启动脚本的资料包，不含 `cpe_test.exe`、`ctsTraffic.exe` 或 iperf3。
> 开箱即用请下载 GitHub Release 的 `cpe_test-v4.3.1-windows-x86_64.zip`；也可以自行
...
- `ctsTraffic.exe`：Microsoft ctsTraffic 2.0.4.0 x64，随官方 v4.3.1 Windows 包固定捆绑并校验；仅支持 Windows 10 或更高版本。
...
3. 在页面里：填辅测机 IP 点“连接” → 需要的话逐网口填 RX 门限（`1800` 或 `90%`）和发送端 UDP `-b` / `-l`
   → 勾选要测的网口组合、方向、协议和 IP 版本（表头小勾选框整列开关；「全选」连双向和 IPv6 一起勾）
   → 填 TCP `-w` / `-P` 与 UDP `-b` / `-l` / `-w` 多档参数（UDP 三项取组合，留空则不下发该参数）
   → 需要时勾「按链路上限裁剪 UDP -b」（默认不勾）
   → 点“预览任务”确认清单和预计耗时 → 点“开始测试”。
4. 同一页面实时显示每个单元的进度；可“结束并出报告”，跑完后点“打开报告”。

网卡表是空的或少了几块，就把测试网段填进“网段前缀”（默认只列 `192.168.`，多个用
空格分隔），或清空该框列出全部网口，再点“连接”——不必回去改 `config.json`。
