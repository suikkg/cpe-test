...
> 如果你从 Git 仓库下载的是 `cpe_test-v6.2.6-windows-config-docs.zip`，那是一份只含
> 配置、文档和启动脚本的资料包，不含 `cpe_test.exe`、`ctsTraffic.exe` 或 iperf3。
> 开箱即用请下载 GitHub Release 的 `cpe_test-v6.2.6-windows-x86_64.zip`；也可以自行
...
- `ctsTraffic.exe`：Microsoft ctsTraffic 2.0.4.0 x64，随官方 v6.2.6 Windows 包固定捆绑并校验；仅支持 Windows 10 或更高版本。
...
1. 辅测机双击 `start_agent.bat`，首次防火墙提示请选择“允许访问”。保持窗口打开。
   浏览器会自动打开辅测机状态页 `http://127.0.0.1:28802`，要报给主控的 IP 就在那张网卡表里，
   主控连上后还能看到它用的是哪一块、正在让本机做什么。
2. 主控机双击 `start_ui.bat`，浏览器会自动打开 `http://127.0.0.1:28800`。没弹出就手动复制这个地址。
3. 页面左边是七个区，按上到下走：
   - **辅测机**：填对端 IP 点“连接”，两边的网卡表就都出来了。
   - **测试计划**：按页面上方的三个标签依次处理“分配链路与套件 → 编辑套件 → 编辑流量配置”；
     在分配表里勾「哪个链路集合跑哪个套件」；套件、任务（TCP/UDP/PING、方向、IP 版本）
     和 TCP/UDP 档位都在这一页直接编辑。TCP/UDP 任务里的「编辑参数 →」会直达对应配置；
...
4. **进度**页实时显示每个单元的判定；要提前收尾点“停止”（跑完当前单元再出报告），
   跑完后点“打开报告”。**历史运行**页能下载整个运行目录、从结果明细恢复报告、
   或把某一轮的计划装载回来重新执行。

网卡表是空的或少了几块，就改“IPv4 前缀过滤”（默认只列 `192.168.`，多个用逗号分隔），
或清空该框列出全部网口，再点“连接”——不必回去改 `config.json`。
连上了却提示**辅测机无对应网卡**，就是这个前缀把对端的网卡全滤掉了。
插拔网线、改完 IP 之后用“本机”或“辅测机”页上的“重新扫描”，不必整页刷新。

控制台只监听本机回环地址，不会被局域网上的其他机器访问到。“测试计划”页的“导出项目”
可以把整份计划存成 `cpe-ui-project.json` 传阅（不含任何口令）。
