...
- 强化 UDP 并发验收：持续采样双端网卡 RX/TX，按共同有效窗口统计速率与丢包，并保留原始记录和重试历史。
- 新增 iPerf 作业生命周期与所有者隔离，支持并发、幂等重试及可靠清理。
- 改进 ping/iperf 前置检查、失败诊断与 HTML 报告，明确区分执行失败、未评估和真实网络丢包。
- 补强 Windows/macOS 网卡识别及独立监控；发布流程新增格式化、测试和 Clippy 质量门禁。
...
- **TCP/UDP 实时网卡速率** — 运行日志按秒打印 OS 网卡计数器的 `nic-rx`；iperf 自报值单独标为 `iperf`
...
- 每次业务重试都先同步停止本轮 client/server，并等待子进程退出、输出线程回收；只有精确 `request_id` 的 server stop 已确认后，才会在同一端口启动下一轮。停止未确认会立即禁止该端口继续重试，不会赌端口已经释放。
...
报告里的“接收网卡平均/P10/中位/P95/最低/最高”和用于 PASS/FAIL 的 RX/TX 数据始终来自操作系统网卡累计字节计数器（Windows `GetIfTable2`、macOS `netstat -ibn`、Linux sysfs），不是 iperf3 自报速率。iperf3 的发送/接收 Mbps 仍保留在流明细中，只用于诊断。
...
如果本轮选择了 iperf，但所有 iperf 单元都没有产生任何有效速率测量（包括两端缺少 iperf3、旧 agent 缺可靠生命周期能力、所有 server 创建失败或所有 client 都未起流），主控会自动追加短时故障诊断：

- 按失败任务的每个唯一 IPv4 方向固定补跑 32 字节短 Ping，每个方向 3 包；自动诊断不再发送 1600/65500，避免故障时增加等待和无关变量。
- 对失败任务涉及的每块网卡，绑定该网卡 IPv4 源地址 Ping 该接口自己的 IPv4 默认网关，用于区分网卡/载体异常和仅 iperf 搭建异常。
...
可靠灌包生命周期由 agent `/health` 的 `reliable_lifecycle_v1` 能力声明；运行中网卡快照由 `live_nic_progress_v1` 声明。选中的任务包含 iperf 时，主控会在启动任何 server/client 前检查可靠生命周期能力以及两端 iperf3。检查失败时，iperf 单元会原位记录 `SETUP_ERROR / IPERF_PREFLIGHT_FAILED`，不会启动任何 iperf3 进程；已选 Ping 单元和上述自动 Ping 故障诊断仍会继续。只选 Ping 时仍兼容旧 agent，也不要求两端安装 iperf3。
...
每个灌包单元完成并回收进程后，会在 `iperf_outputs/` 保存：
...
HTML 报告底部的“原始输出”仍内嵌便于查看的内容，并提供“独立原始记录”和“网卡逐样本 CSV”链接。`master_*.log` 只保存运行进度、摘要、错误和这些文件的路径，不重复塞入大体积的全部 iperf 原文。复制报告给别人时，应连同整个 `iperf_outputs/` 一起复制。
...
HTML 报告底部仍有“原始输出”。此外，每条 TCP/UDP 流会在 `iperf_outputs/` 独立生成 `iperf_raw_*.log`，所有网卡监控会生成 `nic_samples_*.csv`；重试不会再覆盖前一次 client 原文。`master_*.log` 不重复写全部原始行，只记录进度和文件路径。

### 报告的 RX/P10 是 iperf3 内部速率还是网卡实际速率

是接收端操作系统网卡计数器的实际速率。iperf3 行内的 `0.00-180.00 sec` 只用来识别真实活跃时间，解决 stdout 缓冲导致日志时间差只有几毫秒的问题；它不会参与正式 RX 平均或 P10 数值计算。iperf3 自报发送/接收速率单独保留在报告明细，便于比较 PC/协议栈是否来不及处理。

### 灌包失败重试前会不会先释放端口

会。每轮先停止 client，再按本轮 `request_id` 精确停止 server，并确认进程已经 `wait` 回收、输出 reader 已结束；确认成功后才允许同端口的下一轮 start。若 kill/wait 或远端确认失败，该流立即停止重试并报告资源清理错误，单元末尾还会按 `owner_id` 再做批量补偿清理。
