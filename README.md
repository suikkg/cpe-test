...
- **UDP `-l` 支持全局与按网口两级配置**：全局默认档位仍在执行页设置；「按网口门限与负载」
  可为某块网卡作为发送端时固定 `-l`，留空则走全局/任务配置。网口策略只影响对应发送腿，
  且会覆盖任务配置和全局档位；项目导出也会保留这项策略。
...
- **报告与 Excel 新增发送端 TX 速率**。判定口径**没有变**：PASS/FAIL 仍然只看接收端
  网卡 RX，TX 只作为诊断。加这一列是为了把「链路收不到」和「压根没发够」
  当场分开——RX 不达标时先看它。概览表、方向摘要、逐行明细三处都有；Excel 的概览与
  逐行明细同步，明细里连 TX-P10 一起给出，方便定位发送端问题。
...
- **P10 退出 PASS/FAIL**：合格线只剩「判定窗口的平均速率」一条。RX 平均达到门限时，
  TX、P10、滚动窗口和中间速率波动都不再改写 PASS。门限贴着链路上限设时
...
- **Wi-Fi 互测门限按实际频段组合显示**：两端都是 5G 时只显示一行 `5G ↔ 5G`；每个组合
  分别保存单向和双向并发下的主控→辅测、辅测→主控四个 RX 门限。具体网卡例外统一走
  “按网口门限与负载”，不再维护第二张 Wi-Fi 网口对覆盖表。
- **项目与运行态配置分离**：可导出/导入 `cpe-ui-project.json`，保存链路集合、套件、
  稳定 ID、绑定关系、流量/判定配置和按网口策略；时长与 UDP 裁剪开关因会改变实际测试行为也会保存。
  RESUME、截图、当前进度、历史结果、会话和 token 不进入项目文件；`/api/config` 仍输出可交给 `master --auto` 的扁平执行配置。
...
- **灌不够不再掩盖确凿的不达标**：`OFFERED_LOAD_LOW` 原来只看「发送端 TX-P10 有没有达到
  ...
  现在 RX 平均是唯一的速率合格线：RX 平均低于门限就判 `RATE_FAIL / RX_BELOW_TARGET`，
  发送端 TX-P10 只作为失败背景写入理由；RX 平均达到门限则 PASS，不再被 TX、P10、滚动窗口
  或中间掉速改写。
...
- **统一 RX/TX 速率判定**：没有可用 RX 平均值或 RX 基础采样覆盖不足时仍保持 `NOT_EVALUATED`；一旦 RX 平均值有效且低于目标，TX 采样、滚动窗口或实际负载不足都直接按子网问题记为 `RATE_FAIL`，RX 达标则不再被这些诊断项否决。
...
网卡平均值按每个样本与有效窗口的实际重叠毫秒数加权，覆盖率按有效时间并集计算。P10 和 5 秒滚动窗口继续写入报告作诊断，但不再推翻已经达到门限的 RX 平均；RX 平均低于门限时，TX 相关不足按 `RATE_FAIL / RX_BELOW_TARGET` 归为子网问题。

#### Ping 子网通信与自动故障诊断

常规 Ping 是**子网通信测试**，默认对每个所选方向分别覆盖 32、1600、65500 字节负载。`PASS` 要求三条**同时**成立：0% 丢包（`received == sent`）、平均 RTT ≤ 该档 Avg 门限、最大 RTT ≤ 该档 Max 门限。门限按「有线 / Wi-Fi」×「小包 / 中包 / 大包」六档分别配置（`ping.wired_*` / `ping.wifi_*`），控制台的「Ping 判定门限」表可以逐档覆盖。早期版本的“收到一个 Echo Reply 就算 PASS”已经废弃：那个口径下一条丢 90% 的链路照样 PASS。
...
| `verify` | 已知验收线 | 检查已建立流量的 RX 平均速率；TX、P10、滚动窗口作为诊断，显式配置的 UDP 丢包率仍单独检查；未提供显式或 EVB 自动目标时返回 `NOT_EVALUATED / TARGET_MISSING` |
