...
- **链路集合默认列出全部组合**：同机两块网口之间也是一条真实链路（走桥接/回环），
  ...
  生成的还是原来那批跨机集合，同机组合一条都不会被勾上——从用户角度就是这个按钮没反应。
  现在筛选是一个口径、两处生效；分组按钮还会说明这次重建了几个集合、含不含同机组合。
  角色键同时带上「跨机/同机」前缀，否则 `主控 SGMII1G ↔ 辅测 SGMII2.5G` 和
  `主控 SGMII1G ↔ 主控 SGMII2.5G` 会被并进同一个集合。
- **新建的链路集合当场能编辑**：名称改成卡片上的输入框（原来是「改名」按钮 + `window.prompt`），
  新建时顺手展开网口对供勾选——此前建出来是一张空卡片，没有任何编辑入口。
  「收起网口对」按钮也真的会收起了，此前两种文案都只把开关拧到展开。
- **分配表加整列开关**：表头勾一下让所有链路都跑这个套件、再勾一下全部取消，
  和高级矩阵的整列开关同一个读法。套件多起来后原本要逐格点 `集合数 × 套件数` 下。
- **UDP `-l` 只有一个来源**：「网口与策略」里按网口的 UDP `-l` 一列去掉，报文长度统一在
  套件的参数配置里设置。全局档位也不再反向覆写套件的默认参数配置——否则「基线 TCP+UDP」
  会一直显示出厂 `udp_profiles` 里那条 `-l 64`，而界面上没有任何地方交代是谁改的。
  （`config.json` 的 `link_profiles.by_nic.udp_length` 仍然可用，界面不再回填也不再下发。）
...
- **agent 认证提到读请求体之前，请求体上限 100 MiB → 1 MiB**：此前请求体在校验 token
  **之前**就被完整读进内存，且 `from_utf8_lossy(..).into_owned()` 又拷一份，未认证的同网段
  ...
  都是 KB 级 JSON 控制消息，1 MiB 已是最大合法请求的上千倍。
  **`http_client::MAX_RESPONSE_BYTES` 保持 100 MiB 不动**：它管的是相反方向，截图
  （`ScreenshotOut.image_b64`，PNG 再 base64 膨胀 4/3，2K/4K 桌面可达 7 MiB）、iperf 输出、
  网卡样本全走响应侧。原注释写着「和 agent 的 MAX_BODY 对齐」，照它去"恢复对齐"会让高分屏
  机器的截图静默失败；注释已改写，并加了一条测试把两者的数量级差距钉死。
  余量已实测：跑完一轮完整测试，agent 侧最大请求体是 `/iperf/server/start` 的 **191 字节**；
  另有一条测试对**每一种** `*Req` 构造放大过的最坏值（160 字节上限的 id、带 zone 的 IPv6、
  加倍的 iperf 参数），最大 6355 字节 = 上限的 1/165，并要求任何请求都保持 16 倍以上余量——
  将来谁往请求结构里塞了批量字段会当场红，而不是等现场截断。
- **PING 单元的预计耗时修正**：旧公式 `count + 5` 漏了收尾等待——最后一个包没回来时
  BSD ping 还要再等约 10 秒。实测 count=5/20/40 分别是 15.0/30.1/50.2 秒，正好
  `count + 10`，即旧公式每个 PING 单元稳定少算 5 秒；ping 次数提到 180 之后这个
  偏差的绝对值也跟着放大。现在按 `count + 12` 估算。（Windows 的 `ping` 对每个没
  回来的包都要等满 4 秒，100% 丢包的单元实际会跑到 `count × 4` 秒——那是故障路径、
  事前无法预测，估算里不假装知道；执行侧的超时预算本来就按这个上限留的。）
- **新增全量预设**：`configs/config-full-tcp-udp-ping.json`（命令行）与
  `projects/cpe-ui-project-full.json`（图形控制台「导入测试项目」）。TCP + UDP + PING
  一次跑全，覆盖 5 块网口两两组合的全部 10 对（6 跨机 + 4 同机）、A→B / B→A / 双向并发、
  IPv4 + IPv6；参数照搬一次实测（TCP `-w 64k/4m -P 10`，UDP `-b 1000m/2500m -l 14k -w 256m`，
...
- **项目文件的边界是明确的**：绑定目前只支持 `mode: "replace"`，手写或导入
  ...
  不会出现“界面显示选了配方、实际却用了默认值”的情况。链路集合里暂时消失的端点会标成
  **端点已消失**：未参与当前绑定的失效对只提示不阻断，已绑定/选中的失效对必须刷新或移除；
  项目导出会保留这些失效引用，重新导入时不会静默丢掉草稿。
- **服务端计划追踪**：`/api/plan` 返回实际展开的单元、层级 sections/trace、参数覆盖后的预览、
  `plan_hash` 和拓扑指纹；点击“开始测试”时会拒绝过期计划，要求重新预览，避免网口刷新后误跑旧分配。

- **一条腿判不了不再一律掩盖另一条腿的 FAIL**：聚合优先级里 `NOT_EVALUATED` 排在
  `RATE_FAIL` 前面，于是双向单元里 B→A 明明判了不达标，只要 A→B 没判成，单元就报
  「无法评价」，概览的失败数里也看不到它。现在按**为什么判不了**分两类：采样/时间轴
  不可信（覆盖率不足、计数器停滞、有效窗口太短、网卡消失）继续掩盖——两条腿跑在同一段
  时间窗里，那段采样塌了另一条腿的数同样可疑；腿内局部的配置问题
  （`CONFIGURED_LOAD_TOO_LOW` / `OFFERED_LOAD_LOW` / `TARGET_MISSING`）不再掩盖，
  单元按 `RATE_FAIL` 报。名单只放确定安全的三个，其余一律按老行为走：放宽一个码，
  对应的就是一批历史上判「无法评价」的单元变成 FAIL。
- **灌不够不再掩盖确凿的不达标**：`OFFERED_LOAD_LOW` 原来只看「发送端 TX-P10 有没有达到
  门限+5% 余量」，于是「门限 1000、`-b 1000m`、实收 580」被判成 `NOT_EVALUATED`——那 5%
  的发送缺口根本解释不了 420 的接收缺口，补足负载也补不回来，FAIL 在那一轮就已经成立。
  现在这道闸加了前提：**只有接收端确实跟上了发送端**（收到的 ≥ 发出的 95%）才算判不了；
  收到的明显少于发出的，说明缺口在路径上，照常判 `RATE_FAIL / RX_BELOW_TARGET`，并在理由
  里注明「发送端没灌够，但缺口远大于少灌的部分」。要判**达标**仍然必须灌够（`单流 -b ×
  流数 ≥ 门限 × 1.05`），灌不够最好的结果依然是「判不了」，拿不到 PASS。
  顺带消掉了一处两条判定链的分叉：TCP/CTS 路径本来就没有这道闸，同一种情况在 UDP 上被
  报成「判不了」、在 TCP 上却是 FAIL。

- **UDP 参数组**：一轮里常常有几对要 `-b 2500m` 单流、另外几对要 1000m/500m 四流，
  还有的带 `-l` 有的不带，而一份档位是**所有**勾中的配对一起扫的——以前只能分两轮
  跑、出两份报告。现在测试矩阵上方可以定义若干组 UDP 参数，矩阵每行用一个下拉选「用哪一组」，
  第一行「默认」管所有没另选的配对（不新建组时界面和以前一样简单）。
  用「选一组」而不是「逐格覆盖」是刻意的：覆盖是差量语义，每个留空的格子都要回头推理
  「这一格空着等于继承谁」，四个格子就是四次推理，而填错了在界面上看不出来。
  组是完整定义——组里 `-l` 留空就是不下发 `-l`，「有几对带 `-l`、另外几对不带」正是靠
  这一点表达的。随之删掉了「填了却没勾 UDP」「填了却不生效」这类只在差量模型里才需要的规则。
- **UDP 参数只有一个出处**：默认组不再是「执行区那四个输入框」，而是参数组表的第一行——
  它和别的组是同一类东西，分开放会让人看到两种长得完全不同的控件在表达同一件事，还得
  额外解释"默认组"这个特例。参数组表同时搬到了测试矩阵**上方**：它是矩阵那一列的图例，
  图例放在表格下面时，人在矩阵里看到的是一个只有「默认」的下拉，不知道别的选项从哪来。
  下拉里的文字也带上了参数摘要（`多流 · 1000m 500m -l 64 ×4 流`），不展开表就知道选的是
  哪一套；只有默认组时下拉直接置灰。
- **网口固定值在矩阵里就地标注**：某行的发送端网口填了 UDP `-b` 时，那一行的下拉底下直接写
  `A→B 网口固定 1000m`。这条优先级以前要点开「预览任务」才看得见——等点开才发现，
  已经晚了一步。
- **「预览任务」逐单元列出最终生效的参数**：每个单元下面缩进列出每条腿真正下发的
  `-b` / `-l` / `-w` / 流数（双向单元两条腿分开列），其中 `-b` 按 Mbps 显示。
  下发的是精确的 bit/s 整数（`1000m` 变成 `-b 1000000000`，为的是不依赖 iperf3 对 `Gbps`
  这类长后缀的非文档行为），原样打印出来十个零要一个个数，而且抄回输入框会变成 10^9 Mbps
  ——那里的裸数字按 Mbps 算。顺手加了道量纲护栏：`-b` 折合超过 400Gbps 时当场报错并说明
  单位怎么写，而不是拿着天文数字去灌包。这是链路裁剪和「网口 > 参数组」优先级
  都算完之后的值，直接取自要交给 iperf3 的那份 `extra`，不另算一遍。优先级这种东西与其让
  人背下来，不如在跑之前就摆出来——填错了当场看得见，比任何校验都直接。
- **UDP 裁剪上限逐类校准**：`RNDIS` 改为跟自己报的协商速率（实测 3700），不再压到
  CPE 子网那一档 2500——把一块能跑 3.7G 的口当 2.5G 用，灌包强度凭空少三分之一；
  `SGMII2.5G` 的上限从 2500 提到 2600（`cpe_path_ceiling_mbps`），这类口的常规档位就是
  `-b 2.6G`，压在 2500 会把每一轮常规灌包都裁一刀，而裁剪本意是拦离谱值、不是修正正常
  量级。`10GUSB`(NCM) 的 4.2G 是已知驱动显示问题，仍按 10G；Wi-Fi 仍固定 2800、不跟
  会来回跳的 PHY 速率。这张表只影响裁剪和一条 socket 缓冲提示，**不参与 PASS/FAIL**。
- **控制台前端的状态层重做**：矩阵和网口表的唯一事实来源改成两份数据（`ROWS`/`NICROWS`），
  DOM 只是它们的投影——事件只改状态、序列化只读状态。此前两边都是「写进 DOM 再用
  `[data-i]` 选择器读回来」，每加一个字段就要在 renderMatrix / collect / applyPairs 三处
  各写一遍同一件事，靠选择器字符串保持一致。顺带修好一件旧事：「刷新网口」不再清空
  已经勾好的矩阵（按端点对认回来）。后端同步把 `validate_request`、`config_from_request`
  两个各 200 行的函数拆成「全局档位 / 逐对 / 网口策略」三段和 `specs_for_pair`。
- **控制台/状态页弹出的是能打开的地址**：绑到 `0.0.0.0` / `::` 时，此前打印和自动弹给
  浏览器的就是监听地址原文 `http://0.0.0.0:28800?token=…`。`0.0.0.0` 是「所有网卡」的
  通配写法、不是目的地址，Chrome 133 起为堵「0.0.0.0 day」直接拦掉对它的请求；而口令
  就在那串 URL 里，人得先看懂要把主机名换掉才进得去。现在通配地址显示成对应的回环，
  另加一行「从别的电脑访问怎么改」。
- **控制台页面打开就接上正在跑的那轮**：进度日志和 `running` 都是控制台进程里的全局
  状态，`/api/progress` 谁来问都是同一份，但轮询此前只在点「开始测试」时才起——从别的
  电脑连过来、本机再开一个标签页、甚至只是按了 F5，看到的都是一个空白进度区，而测试
  正跑着。现在页面加载时接一次：在跑就续着看，跑完了也把报告按钮留住。
  （同时刻只允许一轮测试这件事没变：别的页面点「开始测试」会被 `running` 闸门当场
  拒绝，不会打断正在跑的那轮。）
- **config.json 可以导回界面**：「下载 config.json」一直是单向的，改完一堆门限和档位，
  下次打开控制台又得从头点一遍。新增「导入 config.json」——门限、档位、配对与方向全部
  回填到原处，只回填不执行。文件里表示不了的东西（`pairs` 自动配对、当前网口表里不存在
  的端点）逐条点名而不是默默少勾一行；配对反着写（`src`/`dst` 调过来）会合并进同一行并
  把方向和双向门限一起翻过来；还没连上辅测机时配对先存着，连上再勾。
  顺带修了回填的两处失真：一档 `-b` 会因为每个 `-l` 档位各生成一份 profile，
  照抄进输入框会让「下载 → 导入」每轮翻一倍；ping 的次数和包长只落在 `tests[]` 上，
  只读 `ping.count` 会把「50 次 × 64 字节」读成默认的「100 次 × 三档包长」，单元数三倍。
- **速率监控支持多块网卡**：网卡列表改成多选，主控和辅测机的口列在一起，最多同时 8 块
  （服务端本来就是按会话存的，`MONITOR_MAX_SESSIONS` 就是 8）。一条链路两端的 TX/RX、
  或者几块口同时灌包时谁在抢带宽，都要放在同一张图上按同一个纵轴比才看得出来。
  每块网卡一个颜色、RX 实线 TX 虚线，图下每块一行读数；某一路自己断了只标记那一条，
  其余继续画。取样接口改成一次问完全部（`POST /api/monitor/samples` 收一组游标）：
  八路各发一个请求会占满浏览器对同一源的并发连接，运行日志那一路开始一秒一顿。
- **不让界面吃掉灌包时的 CPU**：曲线按像素列做 min/max 压缩（8 条曲线 × 7200 点 × RX/TX
  = 十一万多条线段，每秒重画一次，而这台机器同时在灌包）；压 min/max 而不是抽稀，
  只持续一两拍的掉速必须保得住。macOS 上多路监控共用一次 `netstat -ibn`：那边每个采样点
  都要 fork 一个子进程，八块网卡就是每秒八个，而这份开销正好落在灌包最忙的时候。
- **文档与示例配置对齐代码**：`config.example.json` 里的 `cpe_path_ceiling_mbps` 仍写着旧的
  2500，README/使用说明里也还留着「RNDIS 按 2.5G 裁、WiFi 取协商速率与 2.5G 的较小值」
  这类已经不成立的说法——那份示例是给人整份抄走的，抄一份就等于悄悄撤销一次校准。
  三处已同步到现值，并补上 README 字段表里一直缺的两个 WiFi 上限；新增一条测试把
  示例配置里的参考值钉在代码默认值上，下次改默认值时它会把这份文件一起拽上。
- **回归覆盖**：单元测试 452 → 490。
...
`config-10gusb.json`、`config-all-common.json` 和 `config-full-tcp-udp-ping.json`；
用 `--config` 或 `start_master_select_config.bat` 选择对应文件即可。需要自定义时复制
最接近的一份再改，不要往 JSON 添加注释。

`config-full-tcp-udp-ping.json` 是全量预设：TCP + UDP + PING 一次跑全，覆盖 5 块网口
两两组合的全部 10 对（6 跨机 + 4 同机）、三种方向、IPv4 + IPv6，参数照搬一次实测
（TCP `-w 64k/4m -P 10`，UDP `-b 1000m/2500m -l 14k -w 256m`，PING `n=180`）。
展开约 210 个单元、预计 11.5 小时。图形控制台可直接导入等价的
`projects/cpe-ui-project-full.json`，拿到同一套链路集合、套件和参数配置。
...
- RNDIS 按它自己报的协商速率裁剪（实测约 3.7G）：这块口报的值是可用的。
- SGMII2.5G 上限 2600 Mbps（`cpe_path_ceiling_mbps`，容得下常规档位 `-b 2.6G`），SGMII1G 上限约 1000 Mbps；WiFi **不跟协商速率**（那是会来回跳的 PHY 速率），5G/6G/频段未识别按 2800 Mbps、2.4G 按 574 Mbps。
...
| `cpe_path_ceiling_mbps` | 2600 | SGMII2.5G/受限 CPE 路径的负载上限，不是 PASS 目标（RNDIS 跟自己的协商速率，不走这一档）|
| `wifi_payload_ceiling_mbps` | 2800 | WiFi 5G/6G（及频段未识别）的负载上限，**不跟协商速率**：那是会来回跳的 PHY 速率 |
| `wifi_24g_payload_ceiling_mbps` | 574 | WiFi 2.4G 单列的负载上限；和 5G 共用 2800 等于对 2.4G 完全不裁剪 |
...
默认按整条路径的可信负载上限裁剪：SGMII2.5G 2600、SGMII1G 约 1G、RNDIS 跟自己的协商速率，WiFi 按固定档位（5G/6G 2800、2.4G 574），不跟会来回跳的 PHY 速率。关掉 `limit_udp_by_link_speed` 可以强制生成，但可能只是在制造发送端拥塞，不建议作为正式验收口径。
