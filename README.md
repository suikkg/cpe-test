...
- ctsTraffic 平台门禁改为真实 Windows 版本检查：主控和 agent 只在确认系统 major 版本不低于 10 时启用 CTS；Windows 7/8/8.1、版本无法确认以及 macOS/Linux 都会 fail-closed，不阻断已选的 iperf3/Ping。
- CTS 参数不再被静默跳过或修正：配置文件中的非法 TCP/UDP `window`、UDP `bandwidth`/`length`、`streams`、`duration`（包括 `duration=0`）会明确记录 `SETUP_ERROR / CTSTRAFFIC_ARGS_INVALID`，且不启动 CTS 进程；交互式输入越界时会要求重新输入。
- CTS 已有工具测量后，server 在显式停止前异常退出或超时会记录 `RATE_FAIL / CTSTRAFFIC_RUNTIME_ERRORS`，不再误归为搭建错误。
- iperf3 UDP profile 支持可选 `window`，会生成 `-w <size>`；UDP 单流硬连通门槛统一覆盖 iperf3 与 ctsTraffic，单向 1 流或双向每方向 1 流均按方向独立尝试至少 3 次，安全耗尽后直接硬失败。
- Windows Release 包固定捆绑并校验 ctsTraffic 2.0.4.0；三平台产物使用互不冲突的归档名和 SHA-256 清单。自动测试不能替代现场网卡、驱动、防火墙与 CPE 环境，发布后仍建议在两台真实 Windows 10+ 设备上完成 CTS TCP/UDP 双机数据流验收。
...
ctsTraffic.exe        ← v4.2.1 Windows Release 已捆绑（仅 Windows 10+）
...
- `streams` 映射为 client 的 `Connections:N`；一个 CTS 进程承载 N 条连接，不会像 iperf3 UDP 那样展开为 N 个进程。CTS 自动化要求 `streams` 在 `1..=32`；配置文件超出范围会记录 `SETUP_ERROR / CTSTRAFFIC_ARGS_INVALID`，交互式输入则会要求重输，不会静默夹到边界值。
...
- `udp_profiles[].bandwidth` 对 CTS 是**每条连接**的 `BitsPerSecond`，总提供速率约为“每流带宽 × streams”；`length` 映射为 `DatagramByteSize`，且不得大于 65507 字节。无法解析或越界的 `bandwidth`/`length` 会记录 `SETUP_ERROR / CTSTRAFFIC_ARGS_INVALID`。默认 `FrameRate=100`、`BufferDepth=1`，属于 ctsTraffic MediaStream 模型，不等同于 iperf3 UDP flood。
...
官方 v4.2.1 Windows Release ZIP 已捆绑固定且校验过的 ctsTraffic 2.0.4.0，但由于发行包差异不内置 iperf3。
...
仓库同时跟踪一份不含可执行程序的
[`cpe_test-v4.2.1-windows-config-docs.zip`](dist/cpe_test-v4.2.1-windows-config-docs.zip)，
